   ...
   /* function haha() {
        $("#haha").html("<font color='red '>哈哈</font>");
    }*/
    /**
     *获取店铺类别列表以及区域信息列表
     *
   ...
   /*     var url=listUrl+'?'+'pageIndex='+pageIndex+'&pageSize='+pageSize+
            '&areaId='+areaId+'&shopCategoryId='+parentId+'&shopName='+shopName;*/
       /* alert("addItems url="+url);*/
        loading=true;
        $.getJSON(url,function(data){
            if(data.success){
                //获取店铺总数
                maxItems=data.count;
                shopList1=data.shopList;
                counnt1=data.count;
               /* alert(" maxItems="+ maxItems);*/
               /* var countnum='<font color="red">'+maxItems+'</font>';
                $("#haha").html(countnum);*/
                var html='';
                //遍历店铺列表,拼接出card集合
                data.shopList.map(function(item,index){
                    html+=''+'<div class="card"  data-shop-id="'
                        ...
                        +'更新</p>'+'<span><a class="button" onclick="jumpshop('+item.shopId+')">点击查看</a></span>'+'</div>'+'</div>';
                });
    ...
    $('.shoplist-button-div').on('click', '.button', function (e) {
        var shopCategoryId = e.target.dataset.categoryId;

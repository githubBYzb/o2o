$(function () {
    ...
    //由于商品添加和编辑是用的是同一页面，该标识用来标明本次是添加还是编辑操作
    var isEdit = false;
    if (productId) {
        //若有productId则为编辑操作
        getInfo(productId);
        ...
        $.getJSON(
            ...
            function (data) {
                if (data.success) {
                    ...
                    $('#normal-price').val(product.normalPrize);
                    $('#promotion-price').val(
    ...
    function getCategory() {
        $.getJSON(categoryUrl, function (data) {
            if (data.success) {
                ...
                var optionHtml = '';
                productCategoryList.map(function (item, value) {
                    optionHtml += '<option data-value="'
                        + item.productCategoryId + '">'
    ...
    //针对商品详情图控件组，若该控件组的最后一个元素发生变化（即上传图片）
